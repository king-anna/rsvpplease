...
Deno.serve(async (req) => {
  ...
  // ---- Cron sweep -------------------------------------------------------
  // Auth against the shared secret: env CRON_SECRET (legacy) OR the value in
  // app_secrets (generated by migration 0010, used by the pg_cron job).
  const provided = req.headers.get("x-cron-secret") || "";
  const envSecret = Deno.env.get("CRON_SECRET");
  let allowed = !!envSecret && provided === envSecret;
  if (!allowed) {
    const { data: row } = await db.from("app_secrets").select("value").eq("key", "cron_secret").maybeSingle();
    allowed = !!row?.value && provided === row.value;
  }
